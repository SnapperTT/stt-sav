...
int main (int argc, char * * argv)
	...
	if (1) {
		...
		while (mgr.doMaintenance(false, false));
		...
		for (auto& it : truth) {
			...
			STTSAV_ASSERT(mgr.loadRecord( key, it.second.rid, blob));
			if (!blob.size() && it.second.blob.size()) {
				archiveId aid = dict.getArchiveId(key);
				printf("missing record! mgr.loadRecord: %i %i %i\n", key.value, aid.value, it.second.rid.value );
				ArchiveFile* AI = mgr.getLazy(aid);
				if (AI) {
					char n[512];
					uint32_t l;
					dict.getArchiveFilename(aid, n, l, 512);
					printf("file: %s %s\n", n, AI->filename.c_str());
					
					for (uint32_t i = 0; i < AI->records.size(); ++i) {
						printf("record %i/%i: %i %i\n", (int)i, (int)AI->records.size(), (int)AI->records[i].key.value, (int)AI->records[i].record.value);
						} 
					abort();
					}
				}
